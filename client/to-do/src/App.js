...
const addItem = async()=>{
  try{
const res = await axios.post('http://localhost:5400/api/item', { item: itemText });
...
useEffect(() => {
  const getItemsList = async () => {
    try {
      const res = await axios.get('http://localhost:5400/api/items');
...
const deleteItem = async (id) => {
  try {
    await axios.delete(`http://localhost:5400/api/item/${id}`);
...
const updateItem = async (e) => {
  e.preventDefault();
  try {
    await axios.put(`http://localhost:5400/api/item/${isUpdating}`, { item: updateItemText });
